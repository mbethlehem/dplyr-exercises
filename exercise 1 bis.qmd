...
here::i_am("dplyr-exercises.Rproj")
library(here)
```
...
knitr::kable(spotify %>% distinct(title, .keep_all=T) %>% distinct(artist, .keep_all=T) %>% group_by(year) %>% summarise("number of artist"= n()), caption= "number of artists in the top spotify over years")
```
## Question 3
...
knitr::kable(spotify %>% distinct(title, artist, .keep_all=T) %>% summarise("minimum" = min(bpm), "maximum" = max(bpm), "mean"= mean(bpm), "median" = median(bpm)), caption = "statistics of the bpm variable")
```
## Question 5
